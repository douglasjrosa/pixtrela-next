...
import { identifyColaboratorByCode } from "./actions";
...
export function KioskHomeClient() {
  ...
  async function handleSubmit(values: { code: number; password: string }): Promise<void> {
    setErrorKey(null);
    setPending(true);
    const result = await identifyColaboratorByCode(values.code, values.password);
    setPending(false);
    if (!result.ok) {
      setErrorKey(result.error);
      return;
    }
    router.replace(`/kiosk/${result.colaboratorId}`);
  }
