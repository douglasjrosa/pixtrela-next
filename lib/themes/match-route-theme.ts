...
export function routeThemeForegroundStyle(
  theme:
    | (Pick<RouteThemeView, "foregroundColor"> &
        Partial<Pick<RouteThemeView, "surfaceColor">>)
    | null,
): {
  color: string;
  ["--foreground"]: string;
  ...
  const preferred = normalizeForegroundColor(theme?.foregroundColor);
  const surface = theme?.surfaceColor
    ? normalizeSurfaceColor(theme.surfaceColor)
    : null;
  const color = surface
    ? pickContrastingForeground(surface, preferred)
    : preferred;
  return {
    color,
    "--foreground": color,
