...
import { useTeamListSelection } from "./team-list-selection-context";

const CHECKBOX_CLASS = cn("size-4 rounded border border-input accent-primary");

export interface TeamListRowCheckboxProps {
  documentId: string;
...
export function TeamListRowCheckbox({
  documentId,
  variant,
  selectAll = false,
  ariaLabel,
}: TeamListRowCheckboxProps) {
  const ctx = useTeamListSelection();
  ...
  if (variant === "table") {
    return (
      <td
        className={cn(
          "relative z-20 w-10 px-2 py-2 text-center align-middle",
        )}
        data-row-select
        onClick={(event) => {
