...
import { useAwardListSelection } from "./award-list-selection-context";

const CHECKBOX_CLASS = cn("size-4 rounded border border-input accent-primary");

export interface AwardListRowCheckboxProps {
  documentId: string;
...
export function AwardListRowCheckbox({
  documentId,
  variant,
  selectAll = false,
  ariaLabel,
}: AwardListRowCheckboxProps) {
  const ctx = useAwardListSelection();
